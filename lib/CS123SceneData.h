...
enum TransformationType {
    TRANSFORMATION_TRANSLATE, TRANSFORMATION_SCALE, TRANSFORMATION_ROTATE,
...
struct CS123SceneMaterial {
    // This field specifies the diffuse color of the object. This is the color
    // you need to use for
    // the object in sceneview. You can get away with ignoring the other color
    // values until
    // intersect and ray.
    //   CS123SceneMaterial() {}
    ...
    void clear() {
        ...
        cReflective.r  = 0.0f; cReflective.g = 0.0f; cReflective.b = 0.0f;
        cReflective.a  = 0.0f;
        cTransparent.r = 0.0f; cTransparent.g = 0.0f; cTransparent.b = 0.0f;
        cTransparent.a = 0.0f;
...
struct CS123SceneTransformation {
    ...
    glm::vec3 translate; // The translation vector. Only valid if transformation
                         // is a translation.
    glm::vec3 scale;     // The scale vector. Only valid if transformation is a
                         // scale.
    glm::vec3 rotate;    // The axis of rotation. Only valid if the
                         // transformation is a rotation.
    float angle;         // The rotation angle in RADIANS. Only valid if
                         // transformation is a
                         // rotation.

    glm::mat4x4 matrix;  // The matrix for the transformation. Only valid if the
                         // transformation is
                         // a custom matrix.
};
